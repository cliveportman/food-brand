<section>

    <div id="contacts-roasting">
      <h2 data-aos="fade-up">Find out what's roasting</h2>
      <div class="sociallinks">

        <a href="#instagram" class="instagram" data-aos="fade-up">
          <svg width="100%" height="100%" viewBox="0 0 236 236" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" xml:space="preserve" xmlns:serif="http://www.serif.com/" style="fill-rule:evenodd;clip-rule:evenodd;stroke-linejoin:round;stroke-miterlimit:2;">
            <g transform="matrix(-8.33333,0,0,8.33333,121.008,141.256)">
                <path d="M0,-5.442C1.503,-5.442 2.722,-4.224 2.722,-2.721C2.722,-1.218 1.503,0 0,0C-1.503,0 -2.722,-1.218 -2.722,-2.721C-2.722,-4.224 -1.503,-5.442 0,-5.442"/>
            </g>
            <g transform="matrix(8.33333,0,0,8.33333,157.328,146.737)">
                <path d="M0,-6.757C-0.541,-6.757 -0.979,-7.195 -0.979,-7.736C-0.979,-8.278 -0.541,-8.716 0,-8.716C0.541,-8.716 0.979,-8.278 0.979,-7.736C0.979,-7.195 0.541,-6.757 0,-6.757M-4.358,0.814C-6.674,0.814 -8.551,-1.063 -8.551,-3.379C-8.551,-5.694 -6.674,-7.571 -4.358,-7.571C-2.043,-7.571 -0.166,-5.694 -0.166,-3.379C-0.166,-1.063 -2.043,0.814 -4.358,0.814M2.006,-8.194C1.858,-8.575 1.682,-8.846 1.396,-9.132C1.109,-9.418 0.838,-9.595 0.457,-9.743C0.169,-9.855 -0.264,-9.988 -1.06,-10.024C-1.92,-10.063 -2.179,-10.072 -4.358,-10.072C-6.538,-10.072 -6.797,-10.063 -7.657,-10.024C-8.453,-9.988 -8.886,-9.855 -9.173,-9.743C-9.555,-9.595 -9.826,-9.418 -10.112,-9.132C-10.397,-8.846 -10.574,-8.575 -10.723,-8.194C-10.835,-7.906 -10.968,-7.473 -11.004,-6.677C-11.043,-5.817 -11.052,-5.558 -11.052,-3.379C-11.052,-1.198 -11.043,-0.941 -11.004,-0.079C-10.968,0.717 -10.835,1.149 -10.723,1.437C-10.574,1.818 -10.397,2.09 -10.112,2.375C-9.826,2.661 -9.555,2.838 -9.173,2.986C-8.886,3.097 -8.453,3.231 -7.657,3.267C-6.797,3.306 -6.538,3.315 -4.358,3.315C-2.179,3.315 -1.92,3.306 -1.06,3.267C-0.264,3.231 0.169,3.097 0.457,2.986C0.838,2.838 1.109,2.661 1.396,2.375C1.682,2.09 1.858,1.818 2.006,1.437C2.118,1.149 2.251,0.717 2.287,-0.079C2.327,-0.941 2.335,-1.198 2.335,-3.379C2.335,-5.558 2.327,-5.817 2.287,-6.677C2.251,-7.473 2.118,-7.906 2.006,-8.194"/>
            </g>
            <g transform="matrix(8.33333,0,0,8.33333,188.636,83.2258)">
                <path d="M0,7.609C-0.04,8.478 -0.178,9.071 -0.38,9.591C-0.588,10.128 -0.867,10.583 -1.321,11.037C-1.774,11.491 -2.23,11.77 -2.768,11.979C-3.287,12.18 -3.88,12.318 -4.749,12.358C-5.62,12.398 -5.897,12.407 -8.115,12.407C-10.332,12.407 -10.61,12.398 -11.481,12.358C-12.351,12.318 -12.943,12.18 -13.463,11.979C-14,11.77 -14.455,11.491 -14.909,11.037C-15.363,10.583 -15.642,10.128 -15.851,9.591C-16.053,9.071 -16.19,8.478 -16.23,7.609C-16.27,6.738 -16.279,6.46 -16.279,4.243C-16.279,2.026 -16.27,1.748 -16.23,0.877C-16.19,0.008 -16.053,-0.586 -15.851,-1.105C-15.642,-1.642 -15.363,-2.097 -14.909,-2.551C-14.455,-3.005 -14,-3.284 -13.463,-3.493C-12.943,-3.695 -12.351,-3.832 -11.481,-3.872C-10.61,-3.912 -10.332,-3.921 -8.115,-3.921C-5.897,-3.921 -5.62,-3.912 -4.749,-3.872C-3.88,-3.832 -3.287,-3.695 -2.768,-3.493C-2.23,-3.284 -1.774,-3.005 -1.321,-2.551C-0.867,-2.097 -0.588,-1.642 -0.38,-1.105C-0.178,-0.586 -0.04,0.008 0,0.877C0.04,1.748 0.049,2.026 0.049,4.243C0.049,6.46 0.04,6.738 0,7.609M4.616,-1.178C4.604,-1.211 4.596,-1.242 4.583,-1.275C3.961,-2.782 2.993,-5.742 3.343,-8.695C3.371,-8.93 3.106,-9.085 2.921,-8.938C2.093,-8.283 0.331,-7.38 -2.275,-8.552L-2.276,-8.549C-9.229,-11.945 -17.638,-9.127 -21.124,-2.196C-23.476,2.476 -22.99,7.829 -20.322,11.914C-20.254,12.005 -20.188,12.097 -20.113,12.185C-18.529,14.032 -16.112,14.752 -13.836,14.294C-14.967,14.86 -16.188,15.14 -17.4,15.151C-16.013,16.276 -14.38,17.174 -12.504,17.732C-6.377,19.554 0.312,16.978 3.615,11.506C6.052,7.469 6.233,2.792 4.616,-1.178"/>
            </g>
          </svg>
        </a>
      
        <a href="#instagram" class="twitter" data-aos="fade-up" data-aos-delay="50">
          <svg width="100%" height="100%" viewBox="0 0 236 236" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" xml:space="preserve" xmlns:serif="http://www.serif.com/" style="fill-rule:evenodd;clip-rule:evenodd;stroke-linejoin:round;stroke-miterlimit:2;">
            <g transform="matrix(8.33333,0,0,8.33333,178.458,142.724)">
                <path d="M0,-6.16C0.007,-6.004 0.011,-5.847 0.011,-5.689C0.011,-0.873 -3.655,4.681 -10.359,4.681C-12.417,4.681 -14.333,4.077 -15.945,3.044C-15.66,3.077 -15.37,3.094 -15.076,3.094C-13.368,3.094 -11.798,2.511 -10.55,1.534C-12.146,1.504 -13.49,0.451 -13.954,-0.997C-13.732,-0.955 -13.504,-0.931 -13.27,-0.931C-12.937,-0.931 -12.614,-0.976 -12.309,-1.06C-13.976,-1.394 -15.232,-2.868 -15.232,-4.633C-15.232,-4.649 -15.232,-4.664 -15.231,-4.679C-14.74,-4.406 -14.179,-4.242 -13.581,-4.223C-14.56,-4.876 -15.203,-5.993 -15.203,-7.257C-15.203,-7.924 -15.022,-8.55 -14.709,-9.089C-12.912,-6.884 -10.226,-5.433 -7.197,-5.281C-7.26,-5.548 -7.292,-5.826 -7.292,-6.112C-7.292,-8.124 -5.659,-9.756 -3.646,-9.756C-2.599,-9.756 -1.651,-9.313 -0.986,-8.605C-0.156,-8.768 0.624,-9.072 1.328,-9.489C1.056,-8.638 0.478,-7.924 -0.274,-7.473C0.462,-7.561 1.165,-7.757 1.818,-8.047C1.33,-7.316 0.712,-6.674 0,-6.16M4.12,5.411C8.709,-0.818 7.446,-9.596 1.254,-14.271C-2.92,-17.422 -8.272,-17.907 -12.77,-16.015C-12.872,-15.965 -12.975,-15.916 -13.074,-15.858C-15.177,-14.633 -16.318,-12.384 -16.276,-10.062C-16.63,-11.276 -16.687,-12.528 -16.479,-13.723C-17.836,-12.559 -19.012,-11.115 -19.897,-9.369C-22.79,-3.668 -21.458,3.374 -16.667,7.606C-13.134,10.728 -8.565,11.747 -4.369,10.868C-4.335,10.863 -4.303,10.86 -4.268,10.853C-2.674,10.512 0.412,10.091 3.254,10.965C3.479,11.035 3.68,10.803 3.569,10.594C3.073,9.662 2.501,7.767 4.122,5.413L4.12,5.411Z"/>
            </g>
        </svg>
        </a>
      
        <a href="#instagram" class="facebook" data-aos="fade-up" data-aos-delay="100">
          <svg width="100%" height="100%" viewBox="0 0 243 236" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" xml:space="preserve" xmlns:serif="http://www.serif.com/" style="fill-rule:evenodd;clip-rule:evenodd;stroke-linejoin:round;stroke-miterlimit:2;">
            <g transform="matrix(8.33333,0,0,8.33333,237.301,141.895)">
                <path d="M0,-7.046C-0.039,-7.152 -0.077,-7.259 -0.125,-7.364C-1.134,-9.579 -3.257,-10.941 -5.57,-11.132C-4.327,-11.363 -3.076,-11.293 -1.907,-10.967C-2.929,-12.433 -4.248,-13.749 -5.896,-14.806C-11.277,-18.256 -18.418,-17.638 -23.11,-13.297C-26.571,-10.095 -28.043,-5.652 -27.592,-1.389C-27.589,-1.355 -27.59,-1.322 -27.586,-1.287C-27.406,0.334 -27.299,3.446 -28.454,6.185C-28.546,6.404 -28.335,6.626 -28.116,6.537C-27.139,6.137 -25.195,5.758 -23.017,7.607L-23.015,7.605C-21.125,9.315 -18.896,10.409 -16.567,10.92L-16.567,1.379L-19.9,1.379L-19.9,-2.415L-16.567,-2.415L-16.567,-5.307C-16.567,-8.597 -14.607,-10.414 -11.609,-10.414C-10.173,-10.414 -8.671,-10.158 -8.671,-10.158L-8.671,-6.927L-10.326,-6.927C-11.957,-6.927 -12.466,-5.916 -12.466,-4.878L-12.466,-2.415L-8.824,-2.415L-9.406,1.379L-12.466,1.379L-12.466,11.21C-9.027,10.953 -5.673,9.461 -3.144,6.732C0.411,2.896 1.431,-2.381 0,-7.046"/>
            </g>
        </svg>
        </a>
      
      </div>
    </div>

    <div id="contacts-contact">
      <h2>Join the <strong>Chickpea</strong> <span>movement</span></h2>
      <p>For trade enquiries,<br>please email</p>
      <a href="mailto:orders@cheekyps.co.uk">orders@cheekyps.co.uk</a>
    </div>

</section>

<style>

section {
  position: relative;
  padding: 0 0 0 10vw; margin-bottom: 1.8vw;
  background: #FA364A;
  display: flex; justify-content: space-between;
}
section:after {
  position: absolute; right: 0; bottom: 0;
  content: '';
  width: 50vw;
  height: 42.5vw;
  background: #F0EEE8;
  border-radius: 50vw 0 0 0;
  -moz-border-radius: 50vw 0 0 0;
  -webkit-border-radius: 50vw 0 0 0;
}

#contacts-roasting {
  width: 24vw;

}
  #contacts-roasting h2 {
  font-family: "Hamilton"; font-size: 6.7vw; text-transform: uppercase; line-height: 0.8em; text-align: left;
  font-weight: 700; letter-spacing: -0.03em;

  }

#contacts-contact {
  position: relative; z-index: 2;
  padding-right: 6.5vw; padding-top: 13vw;
}

#contacts-contact h2 {
  margin-bottom: 0.5vw; ;
  font-family: "Hamilton"; font-size: 7vw; text-transform: uppercase; line-height: 0.7em; text-align: left;
  font-weight: 300; letter-spacing: -0.03em;
}
#contacts-contact h2 strong {
  display: block;
  font-family: "Hamilton"; font-size: 5.2vw; text-transform: uppercase; line-height: 0.9em; text-align: left;
  font-weight: 700; letter-spacing: -0.03em;
}
#contacts-contact h2 span {
  display: block; margin-top: -0.5vw;
  font-family: "Hamilton"; font-size: 5.2vw; text-transform: uppercase; text-align: left;
  font-weight: 500; letter-spacing: -0.03em;
}
#contacts-contact p {
  margin-bottom: 0.5vw;
  font-family: "Hamilton"; font-size: 3vw;  line-height: 1em; text-align: left;
  font-weight: 300; letter-spacing: -0.03em; text-transform: uppercase;

}
#contacts-contact p strong {
  font-weight: 700; letter-spacing: -0.03em; font-size: 3.2vw;
}
#contacts-contact a {
  display: block; width: 100%;
  font-family: "Hamilton"; font-size: 2.4vw;  line-height: 1.1em; text-align: left;
  font-weight: 700; color: inherit; text-decoration: none; letter-spacing: -0.05em;

}
#contacts-contact a:focus, #contacts-contact a:hover, #contacts-contact a:active {
  color: #00BCB5;
}








.sociallinks {
    width: 100%;
    display: flex;
    justify-content: space-between;
    margin: 1.2vw 0 21.6vw;
  }
  
  .sociallinks a {
  display: block; position: relative;
  width: 6vw; height: 6vw;
  transition: top 0.3s;
}

a svg path {
  fill: #1D1E1B; transition: 0.3s;
}  
a:active , a:hover , a:focus  {
  top: -4px;
}

</style>